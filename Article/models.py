...
class New_Article(models.Model):
	...
	def save(self,*args,**kwargs):
		...
		Article_Comment(object_id = self.id)
		return super(New_Article,self).save(*args,**kwargs)

class Article_Comment(models.Model):
	comment_name = models.CharField(max_length= 20,validators = [valid_name],blank = True,verbose_name = "Your Name")	
	comment_date = models.DateField(blank = True)	
	slug = models.SlugField()
	object_id = models.PositiveIntegerField(blank = True)
	comment_content = models.CharField(max_length = 400,validators = [valid_comment],blank = True)
